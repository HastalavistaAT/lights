...
JD2000 = 2451545
h = -50.0/60.0*RAD
B = math.radians(48.275857)       # geographische Breite Erkelenz
...
switchoff = datetime.time(23, 0, 0);
switchrelay(True)
ison = False
time.sleep(5)
while True:
...
#	      format(AMh, AMm, UMh, UMm))
	...
	if (time_in_range(datetime.time(UMh, UMm, 0), switchoff, current_time)):
		if (ison != True):
			ison = True
			switchrelay(False)
	else:
